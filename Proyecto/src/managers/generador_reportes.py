...
class GeneradorReportes:
    """Clase para generar reportes de arrendos y pagos."""
    ...
    def generar_reporte_recaudacion(self, mes: str) -> Dict[str, Any]:
        ...
        # Calcular arrendos
        total_arrendos = sum(a["arre_valor"] for a in arrendos)
        arrendos_pagados = sum(
            a["arre_valor"] for a in arrendos if a["arre_estado"] == "CANCELADO"
        )

        # Calcular servicios
        total_servicios = sum(p["pago_valorTotal"] for p in pagos)
        servicios_pagados = sum(
            p["pago_valorTotal"] for p in pagos if p["pago_estado"] == "CANCELADO"
        )
        ...
        return {
            ...
            "total_pendiente": (total_arrendos - arrendos_pagados)
    ...
    def obtener_resumen_general(self) -> Dict[str, Any]:
        ...
        arrendos_activos = len(
            [a for a in arrendos if a["arre_estado"] in ["PENDIENTE", "CANCELADO"]]
        )
