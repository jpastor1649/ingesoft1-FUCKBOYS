...
class ServicioAcueducto:
    ...
    def crear_detalle(
        ...
        piso: int,
        consumo: float,
        cargo_fijo_acueducto: float,
        tarifa_acueducto: float,
        cargo_fijo_alcantarillado: float,
        tarifa_alcantarillado: float,
        descuento: int,
    ) -> bool:
        """
        ...
        fields = [
            ...
            "acue_piso",
            "acue_consumo",
            "acue_cargoFijoAcueducto",
            "acue_tarifaAcueducto",
            "acue_cargoFijoAlcantarillado",
            "acue_tarifaAlacantarillado",
            "acue_descuento",
        ]
        values = (
            reci_id,
    ...
    def calcular_distribucion_apartamentos(
        ...
        for apar_id, factor in factores_distribucion.items():
            ...
            distribucion[apar_id] = {
                ...
                "valor_alcantarillado": round(
                    calculo_total["valor_alcantarillado"] * factor, 2
                ),
    ...
    def obtener_estadisticas_tarifas(self) -> Dict[str, Any]:
        ...
        tarifas_alcantarillado = [
            float(d["acue_tarifaAlacantarillado"]) for d in detalles
        ]
        cargos_fijos_acueducto = [float(d["acue_cargoFijoAcueducto"]) for d in detalles]
        cargos_fijos_alcantarillado = [
            float(d["acue_cargoFijoAlcantarillado"]) for d in detalles
        ]
        descuentos = [int(d["acue_descuento"]) for d in detalles]

        return {
            "total_registros": len(detalles),
            "tarifa_acueducto_promedio": round(
                sum(tarifas_acueducto) / len(tarifas_acueducto), 4
            ),
            "tarifa_alcantarillado_promedio": round(
                sum(tarifas_alcantarillado) / len(tarifas_alcantarillado), 4
            ),
            "cargo_fijo_acueducto_promedio": round(
                sum(cargos_fijos_acueducto) / len(cargos_fijos_acueducto), 2
            ),
            "cargo_fijo_alcantarillado_promedio": round(
                sum(cargos_fijos_alcantarillado) / len(cargos_fijos_alcantarillado), 2
            ),
