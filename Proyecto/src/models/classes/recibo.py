...
class Recibo:
    ...
    def crear(
        ...
        if not self._validar_datos(
            fecha, servicio, mes, consumo_inicial, consumo_final
        ):
            return None
    ...
    def obtener_por_apartamento(self, apar_id: int) -> List[Dict[str, Any]]:
        ...
        reci_ids = [c["corre_reci_id"] for c in correspondencias]
        print(
            f"[DEBUG] Recibos asociados al apartamento {apar_id}: {reci_ids}"
        )  # <-- Debug
    ...
    def obtener_por_mes_y_servicio(
    ...
    def eliminar(self, reci_id: int) -> bool:
        ...
        if self._tiene_dependencias(reci_id):
            print(
                f"❌ No se puede eliminar: el recibo {reci_id} tiene registros asociados"
            )
            return False
    ...
    def asociar_apartamento(self, reci_id: int, apar_id: int) -> bool:
        ...
        if self._existe_correspondencia(reci_id, apar_id):
            print(
                f"❌ Ya existe asociación entre recibo {reci_id} y apartamento {apar_id}"
            )
            return False
    ...
    def desasociar_apartamento(self, reci_id: int, apar_id: int) -> bool:
        ...
        if not self._existe_correspondencia(reci_id, apar_id):
            print(
                f"❌ No existe asociación entre recibo {reci_id} y apartamento {apar_id}"
            )
            return False
    ...
    def obtener_estadisticas_mes(self, mes: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "servicios_faltantes": list(
                set(self.SERVICIOS_VALIDOS) - servicios_disponibles
            ),
            "consumo_total_mes": round(consumo_total, 3),
            "porcentaje_completitud": (
                round((len(servicios_completos) / len(recibos)) * 100, 2)
                if recibos
                else 0
            ),
        }
