...
class Lectura:
    ...
    def crear(self, apar_id: int, fecha: str, servicio: str, mes: str,
        ...
        """
        if not self._validar_datos(apar_id, fecha, servicio, mes, consumo_inicial, consumo_final):
            return False
            
        # Verificar unicidad
        if self.obtener_por_clave(apar_id, fecha, servicio):
            print(f"❌ Ya existe lectura para apartamento {apar_id}, fecha {fecha}, servicio {servicio}")
            return False
            
        fields = ['lec_apar_id', 'lec_fecha', 'lec_servicio', 'lec_mes', 
                 'lec_consumoInicial', 'lec_consumoFinal']
    ...
    def obtener_por_clave(self, apar_id: int, fecha: str, servicio: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def obtener_por_apartamento_mes_servicio(self, apar_id: int, mes: str, servicio: str) -> List[Dict[str, Any]]:
        """
    ...
    def actualizar(self, lec_id: int, consumo_inicial: float, consumo_final: float) -> bool:
        """
    ...
    def obtener_consumo_total_mes_servicio(self, mes: str, servicio: str) -> float:
        ...
        for lectura in lecturas_servicio:
            consumo = float(lectura['lec_consumoFinal']) - float(lectura['lec_consumoInicial'])
            total += consumo
            
        return total
    
    def obtener_consumos_apartamento_mes(self, apar_id: int, mes: str) -> Dict[str, float]:
        """
    ...
    def calcular_factor_consumo_apartamento(self, apar_id: int, mes: str, servicio: str) -> float:
        ...
        lecturas_apartamento = self.obtener_por_apartamento_mes_servicio(apar_id, mes, servicio)
        if not lecturas_apartamento:
            return 0.0
            
        consumo_apartamento = 0.0
        for lectura in lecturas_apartamento:
            consumo_apartamento += float(lectura['lec_consumoFinal']) - float(lectura['lec_consumoInicial'])
    ...
    def obtener_historial_consumos_apartamento(self, apar_id: int, limit: int = 12) -> List[Dict[str, Any]]:
        """
    ...
    def _validar_datos(self, apar_id: int, fecha: str, servicio: str, mes: str,
                      consumo_inicial: float, consumo_final: float) -> bool:
        """
