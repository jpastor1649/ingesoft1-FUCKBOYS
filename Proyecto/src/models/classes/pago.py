...
class Pago:
    ...
    def crear(self, lec_apar_id: int, lec_fecha: str, lec_servicio: str, mes: str,
              tipo_lectura: str, consumo: int, valor_total: int, 
              estado: str = 'PENDIENTE', fecha_pago: str = None, observacion: str = "") -> bool:
        """
        Crear un nuevo pago
        ...
        """
        if not self._validar_datos(lec_apar_id, lec_fecha, lec_servicio, mes, 
                                  tipo_lectura, consumo, valor_total, estado):
            return False
            
        # Verificar que no exista ya el pago
        if self.obtener_por_clave(lec_apar_id, lec_fecha, lec_servicio):
            print(f"❌ Ya existe pago para apartamento {lec_apar_id}, fecha {lec_fecha}, servicio {lec_servicio}")
            return False
        ...
        fields = ['pago_lec_apar_id', 'pago_lec_fecha', 'pago_lec_servicio', 'pago_mes',
                 'pago_fechaPago', 'pago_tipoLectura', 'pago_consumo', 'pago_valorTotal',
                 'pago_estado', 'pago_observacion']
        values = (lec_apar_id, lec_fecha, lec_servicio, mes.upper(), fecha_pago,
                 tipo_lectura, consumo, valor_total, estado, 
    ...
    def obtener_por_clave(self, lec_apar_id: int, lec_fecha: str, lec_servicio: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def obtener_por_mes(self, mes: str) -> List[Dict[str, Any]]:
        ...
        """
        where = f"pago_mes = '{mes.upper()}' ORDER BY pago_lec_apar_id, pago_lec_servicio"
        return self.connector.get_filtered(where)
    ...
    def obtener_apartamento_mes_servicio(self, apar_id: int, mes: str, servicio: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def actualizar_estado(self, lec_apar_id: int, lec_fecha: str, lec_servicio: str,
                         nuevo_estado: str, fecha_pago: str = None) -> bool:
        """
    ...
    def registrar_pago(self, lec_apar_id: int, lec_fecha: str, lec_servicio: str, fecha_pago: str) -> bool:
        """
        Registrar pago de un servicio
        """
        return self.actualizar_estado(lec_apar_id, lec_fecha, lec_servicio, 'CANCELADO', fecha_pago)
    ...
    def actualizar_valor(self, lec_apar_id: int, lec_fecha: str, lec_servicio: str, nuevo_valor: int) -> bool:
        """
    ...
    def calcular_total_apartamento_mes(self, apar_id: int, mes: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'estado_general': 'PENDIENTE' if pendientes > 0 else 'CANCELADO' if cancelados > 0 else 'SIN_PAGOS'
        }
    ...
    def obtener_resumen_mes(self, mes: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'porcentaje_recaudo': round((total_pagado / total_esperado) * 100, 2) if total_esperado > 0 else 0,
            'servicios_por_apartamento': {apar_id: len(pagos) for apar_id, pagos in apartamentos.items()}
        }
    ...
    def obtener_historial_pagos_apartamento(self, apar_id: int, limit: int = 12) -> List[Dict[str, Any]]:
        """
        Obtener historial de pagos de un apartamento
        """
        where = f"pago_lec_apar_id = {apar_id} ORDER BY pago_lec_fecha DESC LIMIT {limit}"
        return self.connector.get_filtered(where)
    ...
    def generar_recibo_apartamento_mes(self, apar_id: int, mes: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'estado_general': self._determinar_estado_recibo(pagos_servicios, arriendo_mes)
        }
    
    def _validar_datos(self, lec_apar_id: int, lec_fecha: str, lec_servicio: str, mes: str,
                      tipo_lectura: str, consumo: int, valor_total: int, estado: str) -> bool:
        """
        ...
        if tipo_lectura not in self.TIPOS_LECTURA_VALIDOS:
            print(f"❌ Tipo de lectura inválido. Debe ser uno de: {', '.join(self.TIPOS_LECTURA_VALIDOS)}")
            return False
        ...
        if estado not in self.ESTADOS_VALIDOS:
            print(f"❌ Estado inválido. Debe ser uno de: {', '.join(self.ESTADOS_VALIDOS)}")
            return False
    ...
    def _determinar_estado_recibo(self, pagos_servicios: Dict[str, Any], arriendo: Optional[Dict[str, Any]]) -> str:
        """
        Determinar estado general del recibo
        """
        servicios_pendientes = pagos_servicios['servicios_pendientes']
        arriendo_pendiente = arriendo and arriendo['arre_estado'] == 'PENDIENTE' if arriendo else False
        
        if servicios_pendientes > 0 or arriendo_pendiente:
            return 'PENDIENTE'
        elif pagos_servicios['servicios_pagados'] > 0 or (arriendo and arriendo['arre_estado'] == 'CANCELADO'):
            return 'CANCELADO'
        else:
            return 'CERRADO'
    ...
    def obtener_estadisticas_generales(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'apartamentos_con_pagos': len(apartamentos),
            'porcentaje_recaudo': round((total_pagado / (total_pendiente + total_pagado)) * 100, 2) if (total_pendiente + total_pagado) > 0 else 0
        }