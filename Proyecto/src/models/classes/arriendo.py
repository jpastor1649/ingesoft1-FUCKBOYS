...
class Arriendo:
    ...
    def crear(self, inq_id: int, apar_id: int, fecha_inicio: str, fecha_fin: str, 
              mes: str, valor: int, estado: str = 'PENDIENTE', 
              fecha_pago: str = None, observaciones: str = "") -> bool:
        """
        ...
        Args:
            ...
            valor: Valor del arriendo
            estado: Estado del arriendo
            fecha_pago: Fecha de pago (opcional)
            observaciones: Observaciones adicionales
        """
        if not self._validar_datos(inq_id, apar_id, fecha_inicio, fecha_fin, mes, valor, estado):
            return False
            
        # Verificar que no exista conflicto con otro arriendo
        if self._existe_conflicto_fechas(apar_id, fecha_inicio, fecha_fin):
            print(f"❌ Conflicto de fechas: el apartamento {apar_id} ya tiene arriendo en ese período")
            return False
            
        fields = ['arre_inq_id', 'arre_apar_id', 'arre_fechaInicio', 'arre_fechaFin', 
                 'arre_mes', 'arre_valor', 'arre_fechaPago', 'arre_estado', 'arre_observaciones']
        values = (inq_id, apar_id, fecha_inicio, fecha_fin, mes.upper(), valor, 
                 fecha_pago, estado, observaciones.strip() if observaciones else None)
        
        affected = self.connector.insert(fields, values)
        return affected > 0
    
    def obtener_por_id(self, fecha_inicio: str, apar_id: int, inq_id: int) -> Optional[Dict[str, Any]]:
        """
        Obtener un arriendo por su clave primaria compuesta
        """
    ...
    def obtener_activos(self) -> List[Dict[str, Any]]:
        """
        Obtener arrendos activos (estados PENDIENTE y CANCELADO)
        """
        where = "arre_estado IN ('PENDIENTE', 'CANCELADO') ORDER BY arre_fechaInicio DESC"
        return self.connector.get_filtered(where)
    ...
    def actualizar_estado(self, fecha_inicio: str, apar_id: int, inq_id: int, 
                         nuevo_estado: str, fecha_pago: str = None) -> bool:
        """
        Actualizar estado de un arriendo
        """
        ...
        # Construir WHERE manualmente para clave compuesta
    ...
    def registrar_pago(self, fecha_inicio: str, apar_id: int, inq_id: int, fecha_pago: str) -> bool:
        """
        Registrar pago de un arriendo
        """
        return self.actualizar_estado(fecha_inicio, apar_id, inq_id, 'CANCELADO', fecha_pago)
    
    def cerrar_arriendo(self, fecha_inicio: str, apar_id: int, inq_id: int) -> bool:
        """
        Cerrar un arriendo definitivamente
        """
        return self.actualizar_estado(fecha_inicio, apar_id, inq_id, 'CERRADO')
    
    def actualizar_valor(self, fecha_inicio: str, apar_id: int, inq_id: int, nuevo_valor: int) -> bool:
        """
        Actualizar valor de un arriendo
        """
        if nuevo_valor <= 0:
    ...
    def obtener_ingresos_mes(self, mes: str) -> Dict[str, Any]:
        """
        Obtener ingresos totales por arrendos en un mes
        """
    ...
    def obtener_morosos(self) -> List[Dict[str, Any]]:
        """
        Obtener arrendos en mora (más de 30 días sin pagar)
        """
    ...
    def _validar_datos(self, inq_id: int, apar_id: int, fecha_inicio: str, 
                      fecha_fin: str, mes: str, valor: int, estado: str) -> bool:
        """
        Validar datos del arriendo
        """
        # Validar IDs
        ...
        # Verificar que existan inquilino y apartamento
    ...
    def _existe_conflicto_fechas(self, apar_id: int, fecha_inicio: str, fecha_fin: str) -> bool:
        """
        Verificar si existe conflicto de fechas para un apartamento
        """
        where = f"""
    ...
    def _existe_inquilino(self, inq_id: int) -> bool:
        """
        Verificar si existe un inquilino
        """
    ...
    def obtener_estadisticas_generales(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'arrendos_pendientes': len(pendientes),
            'arrendos_cerrados': len([arr for arr in todos if arr['arre_estado'] == 'CERRADO']),
